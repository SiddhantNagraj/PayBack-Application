...
import { userActionCreator } from '../../../redux/actionCreator/userAction';
 class SettleUp extends React.Component{
  ...
  render(){
        ...
        <span className='tittle2'>Settle up</span>
        <button className = "float-right" onClick = {this.props.friend}><i class="fas fa-times"></i></button>
        </div>

        <div className = "frnd-set">
        <button className='btn3' onClick = {this.PaidBy.bind(this)}>{(this.state.byValue == "you")?"You":this.state.byValue.slice(0,6) + " ..." }</button> paid <button className='btn4' onClick = {this.PaidTo.bind(this)}>{(this.state.toValue == "you" || this.state.toValue == "Select")?this.state.toValue:this.state.toValue.slice(0,6) + " ..."}</button>
        </div>
